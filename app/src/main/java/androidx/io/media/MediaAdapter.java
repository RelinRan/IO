...
public class MediaAdapter extends RecyclerView.Adapter<ViewHolder> {
    ...
    protected int getItemLayoutId(int viewType) {
        if (viewType == ITEM_FILE || viewType == ITEM_AUDIO) {
            return R.layout.io_item_file;
        }
        if (viewType == ITEM_IMAGE || viewType == ITEM_VIDEO) {
            return R.layout.io_item_image_video;
        }
        return R.layout.io_item_file;
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        if (viewType == ITEM_FILE || viewType == ITEM_AUDIO) {
            ...
            iv_check.setImageResource(item.isCheck() ? R.mipmap.io_item_check : R.mipmap.io_item_uncheck);
    ...
    private void showIconThumbnail(ImageView icon, ImageView tag, TextView duration, File file, int position) {
        ...
        if (file.isDirectory()) {
            icon.setImageResource(R.mipmap.io_dir);
        } else {
            icon.setImageResource(R.mipmap.io_none);
            ...
            if (MediaProvider.isAudio(suffix)) {
                ...
                if (item.getThumbnail() == null) {
                    icon.setImageResource(R.mipmap.io_music);
                } else {
            ...
            if (MediaProvider.isDoc(suffix)) {
                icon.setImageResource(R.mipmap.io_doc);
            }
            if (MediaProvider.isPPT(suffix)) {
                icon.setImageResource(R.mipmap.io_ppt);
            }
            if (MediaProvider.isPDF(suffix)) {
                icon.setImageResource(R.mipmap.io_pdf);
            }
            if (MediaProvider.isTXT(suffix)) {
                icon.setImageResource(R.mipmap.io_txt);
            }
            if (MediaProvider.isHtml(suffix)) {
                icon.setImageResource(R.mipmap.io_html);
            }
            if (MediaProvider.isJson(suffix)) {
                icon.setImageResource(R.mipmap.io_json);
            }
            if (MediaProvider.isXml(suffix)) {
                icon.setImageResource(R.mipmap.io_xml);
            }
            if (MediaProvider.isContact(suffix)) {
                icon.setImageResource(R.mipmap.io_vcf);
            }
            if (MediaProvider.isCompression(suffix)) {
                icon.setImageResource(R.mipmap.io_zip);
            }
